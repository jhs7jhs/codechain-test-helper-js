...
class BlockSyncMessage {
    ...
    static fromBytes(bytes) {
        ...
        if (msgId === MessageType.MESSAGE_ID_STATUS) {
            ...
            const totalScore = new U256_1.U256(parseInt(msg[0].toString("hex"), 16));
            const bestHash = new H256_1.H256(msg[1].toString("hex"));
            const genesisHash = new H256_1.H256(msg[2].toString("hex"));
            ...
            const id = decodedmsg[1].length === 0
                ? new U256_1.U256(0)
                : new U256_1.U256(parseInt(decodedmsg[1].toString("hex"), 16));
            ...
            case MessageType.MESSAGE_ID_GET_HEADERS: {
                ...
                return new RequestMessage({
                    ...
                    startNumber: bytes[0].length === 0
                        ? new U256_1.U256(0)
                        : new U256_1.U256(parseInt(bytes[0].toString("hex"), 16)),
                    maxCount: bytes[1].length === 0
                        ? new U256_1.U256(0)
                        : new U256_1.U256(parseInt(bytes[1].toString("hex"), 16))
                });
            }
            case MessageType.MESSAGE_ID_GET_BODIES: {
                ...
                return new RequestMessage({
                    type: "bodies",
                    data: bytes.map(buf => new H256_1.H256(buf.toString("hex")))
                });
            }
            case MessageType.MESSAGE_ID_GET_STATE_HEAD: {
            }
